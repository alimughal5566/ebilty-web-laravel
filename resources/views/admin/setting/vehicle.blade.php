...
@section('content')
                ...
                <div class="kt-portlet__body kt-portlet__body--fit-x">
                    <div class="col-lg-12">
                        ...
                        <div class="kt-portlet__head">
                            ...
                            <div class="kt-portlet__head-label" >
                                <h3 class="kt-portlet__head-title">
                                    <button class="btn btn-primary btn-sm" data-toggle="modal" data-target="#add_vehicle"><i class="fas fa-plus"></i></button>
                                </h3>
                            </div>
                        </div>
    ...
    <script>
        ...
        $('#edit_vehicle_form').on('submit', function(e) {
            ...
            $.ajax({
                ...
                success: function( msg ) {
                    ...
                    markup = '<tr><td>  </td><td>'+ msg.row.name + '</td><td>'+ msg.cat_name +'</td><td>'+ toggle_button +'</td><td> <a onclick="delete_vehicle('+ msg.row.id +')" class="btn btn-primary btn-sm"> <i class="fas fa-edit"></i>  </a> <a onclick="edit_vehicle('+ msg.row.id +')" class="btn btn-danger btn-sm"> <i class="far fa-trash-alt"></i>  </a></td></tr>';
                    tableBody = $("#vehicle_table tbody");
                    // tableBody.append(markup);
                    setTimeout(function(){ $("#save_msg").remove(); }, 3000);
                }
        ...
        function make_status_active(id){
            $(this).parents('tr').first().remove();
            var url = '{{route('admin.setting.make_status_active_vehicle')}}'
            $.ajax({
                type: "get",
                url: url,
                data: {id:id},
        ...
        function make_status_inactive(id){
            $(this).parents('tr').first().remove();
            var url = '{{route('admin.setting.make_status_inactive_vehicle')}}'
            $.ajax({
                type: "get",
                url: url,
                data: {id:id},
