...
@section('content')
                ...
                <div class="kt-portlet__body kt-portlet__body--fit-x">
                    <div class="col-lg-12">
                        ...
                        <div class="kt-portlet__head">
                            ...
                            <div class="kt-portlet__head-label" >
                                <h3 class="kt-portlet__head-title">
                                    <button class="btn btn-primary btn-sm" data-toggle="modal" data-target="#add_vehicle_category"><i class="fas fa-plus"></i></button>
                                </h3>
                            </div>
                        </div>
                        ...
                        <div class="kt-portlet__body kt-portlet__body--fit-x">
                            <div class="col-lg-12">
                                <table class="table" id="vehicle_category_table">
                                    <thead>
                                    <tr>
                                        <th scope="col">#</th>
                                    ...
                                    @forelse($types as $type)
                                        <tr>
                                            ...
                                            <td>
                                                <a onclick="edit_vehicle_category({{$type->id}})" class="btn btn-primary btn-sm"> <i class="fas fa-edit"></i>  </a>
                                                <a onclick="delete_vehicle_category({{$type->id}})" class="btn btn-danger btn-sm"> <i class="far fa-trash-alt"></i>  </a>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
    ...
    <!-- end:: Content -->
    <div class="modal fade" id="add_vehicle_category" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Add Vehicle Category</h5>
                                    ...
                                    <div class="kt-section__body">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-lg-9 col-xl-9">
                                                <input class="form-control" type="text" id="name" name="name" placeholder="Enter Vehicle Category" required="">
    ...
    <div class="modal fade" id="edit_vehicle_category_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <form id="edit_vehicle_category_form" method="get" >
                        @csrf
                        <div class="row">
                            <div class="col-xl-2"></div>
                            <div class="col-xl-8">
                                    ...
                                    <div class="kt-section__body">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-lg-9 col-xl-9">
                                                ...
                                                <input class="form-control" type="hidden" id="vehicle_category_id" name="vehicle_category_id" >
                                            </div>
                                        </div>
                                    </div>
                                </div>
    ...
    <script>
        $('#add_payment_form').on('submit', function(e) {
            e.preventDefault();
            var url = '{{route('admin.setting.add_vehicle_type')}}';
            ...
            $.ajax({
                ...
                success: function( msg ) {
                    $("#save_msg").html()
                    $('#add_vehicle_category').modal('hide');
                    $('#save_msg').css('display','block');
                    $("#save_msg").append(msg.msg);
                    var toggle_button = '';
                    var number = $('#vehicle_category_table tr:last').attr('id');
                    console.log(number)
                    if(msg.row.status == 0){
                    ...
                    markup = '<tr><td>  </td><td>'+ msg.row.name + '</td>><td>'+ toggle_button +'</td><td> <a onclick="delete_vehicle_category('+ msg.row.id +')" class="btn btn-primary btn-sm"> <i class="fas fa-edit"></i>  </a> <a onclick="edit_vehicle_category('+ msg.row.id +')" class="btn btn-danger btn-sm"> <i class="far fa-trash-alt"></i>  </a></td></tr>';
                    tableBody = $("#vehicle_category_table tbody");
                    tableBody.append(markup);
                    setTimeout(function(){ $("#save_msg").remove(); }, 3000);
                }
            });
        });
        $('#edit_vehicle_category_form').on('submit', function(e) {
            e.preventDefault();
            var url = '{{route('admin.setting.update_vehicle_category')}}';
            var token = $('#_token').val();
            ...
            $.ajax({
                type: "get",
                url: url,
                data: {id:vehicle_category_id, name:name, status:status},
                success: function( msg ) {
                    $("#save_msg").html()
                    $('#edit_vehicle_category_modal').modal('hide');
                    $('#save_msg').css('display','block');
                    $("#save_msg").append(msg.msg);
                    var toggle_button = '';
                    // var number = $('#vehicle_category_table tr:last').attr('id');
        ...
        function edit_vehicle_category(id) {
            var url = '{{route('admin.setting.edit_vehicle_category')}}'
            $.ajax({
                type: "get",
                url: url,
                data: {id:id},
                success: function( msg ) {
                    $("#save_msg").html()
                    console.log(msg)
                    $('#edit_vehicle_category_modal').modal('show');
                    $('#edit_name').val(msg.name);
                    $('#vehicle_category_id').val(msg.id);
                    setTimeout(function(){ $("#save_msg").remove(); }, 3000);
                }
            });
        }
        function delete_vehicle_category(id) {
            $(this).parents('tr').first().remove();
            var url = '{{route('admin.setting.delete_vehicle_category')}}'
            $.ajax({
                type: "get",
                url: url,
                data: {id:id},
        ...
        function make_status_active(id){
            $(this).parents('tr').first().remove();
            var url = '{{route('admin.setting.make_status_active_vehicle_category')}}'
            $.ajax({
                type: "get",
                url: url,
                data: {id:id},
        ...
        function make_status_inactive(id){
            $(this).parents('tr').first().remove();
            var url = '{{route('admin.setting.make_status_inactive_vehicle_category')}}'
            $.ajax({
                type: "get",
                url: url,
                data: {id:id},
